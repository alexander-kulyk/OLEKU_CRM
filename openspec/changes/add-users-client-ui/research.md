...
- **Confidence:** Medium — the complete PRD, current client, project rules, installed frontend contracts, and the planned Users API were inspected; confidence is below high because the API is not implemented and the identity, authentication, and deployment phone-region inputs needed by mandatory UI behavior do not exist.
- **Blocking unknowns:** 3 — availability of the planned Users API (U-001), current-operator/authentication context (U-002), and deployment default phone-region configuration (U-003).
...
- Render the non-archived, server-paginated Users table with the eight specified columns, explicit status text and status-dependent styling, formatted nullable address/phone, UTC Last Login, accessible Edit action, and narrow-viewport usability (`docs/prd/release 1.0.0/usersPage/usersPage.md:393-540,1024-1097`).
...
- Authentication, authorization, roles, account recovery, user creation/invitation, restoration, privacy erasure, and audit storage remain separately scoped by the PRD (`docs/prd/release 1.0.0/usersPage/usersPage.md:2513-2532`).
...
| RQ-007 | Are identity and deployment phone settings available? | Self-archive, 401/403, and national phone parsing depend on them | Unresolved: none exist | EVID-010, F-010, U-002, U-003 | Full compliance cannot be claimed without explicit integration contracts |
...
| EVID-010 | `client/src/app/App.tsx:1-8`; `client/src/main.tsx:1-5`; bounded repository search on 2026-08-16 | Code + command output | Client has no auth/session/current-operator or deployment phone-configuration provider |
...
- **F-010 [Verified]** Mandatory context is missing. There is no current operator for disabling self-archive, no central auth recovery flow for 401, and no deployment default phone region for equivalent national-phone normalization (EVID-010). Last Login can be rendered deterministically in UTC. The server change explicitly defers self/last-admin enforcement, auth statuses, audit, and rate limiting (`add-users-server-api/proposal.md:38-45`).
...
| R-006 | Phone normalization uses a browser-region guess | F-010 | High with current app | Medium/High | Block ambiguous national-phone validation on U-003 |
...
| U-003 | Unknown | Source and wire shape for the deployment default phone region | Normalized phone dirty/validation behavior can be wrong | Define a deployment-configuration contract; do not infer the region from the browser |
...
- No deployed API, database, user records, credentials, auth service, audit system, or rate-limiter state was accessed; static artifacts cannot prove runtime availability.
