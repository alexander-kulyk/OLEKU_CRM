...
The application SHALL present a navigation menu, positioned along the left edge and laid out vertically, that remains visible on every page. The menu SHALL contain exactly two destinations, in this order: Calendar and Users.

#### Scenario: Menu is present on every page

- **WHEN** the user is on any page of the application
- **THEN** the left vertical menu is visible with the Calendar and Users entries in that order
...
- **THEN** the Calendar entry is shown in its active state and the Users entry is not

#### Scenario: Indication follows navigation

- **WHEN** the user navigates from the calendar page to the Users page
- **THEN** the Users entry becomes active and the Calendar entry ceases to be active
...
The Users destination SHALL render a placeholder page identifying itself and indicating that its content is not yet available. It SHALL NOT request data from the API.
